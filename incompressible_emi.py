...
def main():
    ...
    # Strain energies (material + incompressibility multiplier)
    psi_e_val = psi_extracellular(C) + incompressibility(C, p)
    psi_i_val = psi_intracellular(C, f0=f0) + incompressibility(C, p)
    ...
    # Penalty term for incompressibility stabilization (applied globally using ufl.dx)
    penalty = dolfinx.fem.Constant(mesh, 1e-10)
    Rp = (J - 1) * p_test * ufl.dx - penalty * p * p_test * ufl.dx
    ...
    R = [Ru, Rp]
    K = [
        [ufl.derivative(Ru, u, u_trial), ufl.derivative(Ru, p, p_trial)],
        [ufl.derivative(Rp, u, u_trial), ufl.derivative(Rp, p, p_trial)],
    ]
    states = [u, p]
    ...
    if save_solution:
        shutil.rmtree("incompressible_emi.bp", ignore_errors=True)
        vtx = dolfinx.io.VTXWriter(mesh.comm, "incompressible_emi.bp", [u])
        vtx.write(0.0)
    ...
    if plot_matrix:
        ...
        plt.spy(problem.A[:, :])
        plt.savefig("A_incompressible_emi.png", dpi=300)

    if save_matrix:
        from petsc4py import PETSc

        viewer = PETSc.Viewer().createASCII(
            "incompressible_emi.txt", mode=PETSc.Viewer.Mode.WRITE
        )
